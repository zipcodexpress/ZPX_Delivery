...
use Zpx\HubDispatch\Service;
use Zpx\Infrastructure\Database\Connection;

final class HubDispatchController
{
    public static function handle(Request $request, string $action, string $id = ''): Response
    {
        ...
        try {
            ...
            $expected = match ($action) {
                'stage', 'dispatch', 'accept', 'load' => 'POST',
            ...
            if ($method === 'POST') {
                ...
                $identity->limit('hub-dispatch:' . $user, 100);
            ...
            $body = match ($action) {
                'stage' => $service->stageScan($user, $input, $key),
                'dispatch' => $service->createDispatchCall($user, $input, $key),
