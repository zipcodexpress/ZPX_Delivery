...
type View = 'sending' | 'receiving' | 'operations';
const label = (value: string) => value.toLowerCase().replaceAll('_', ' ').replace(/^./, c => c.toUpperCase());
const date = (value: string) => new Date(value).toLocaleString();

export function Shipping({ profile, audience, onAccount, onLogout }: {
  ...
  const [view, setView] = useState<View>(operations ? 'operations' : 'sending');
  ...
  useEffect(() => {
    ...
    let active = true; setLoading(true); setSelected(null); setTimeline(null); setQuote(null); setPayment(null); setShippingLabel(null); setCreating(false); setError('');
  ...
  return <main className="shipping-page">
    <header><strong>ZipcodeXpress<span className="brand-dot">.</span></strong><nav aria-label="Account"><span>{profile.name}</span><button onClick={onAccount} disabled={busy}>Account</button><button onClick={onLogout} disabled={busy}>Sign out</button></nav></header>
    ...
    <div className="shipping-tabs" aria-label="Shipment views">{(operations ? ['operations'] : ['sending', 'receiving']).map(v => <button key={v} disabled={busy} aria-pressed={view === v} onClick={() => setView(v as View)}>{v === 'operations' ? 'Shipment queue' : label(v)}</button>)}<button className="refresh" disabled={busy || loading} onClick={() => void run(async () => { await reload(); if (selected) await open(selected); })}>Refresh</button></div>
    {error && <p className="error" role="alert">{error}</p>}{notice && <p className="notice" role="status">{notice}</p>}
    <div className="shipping-grid">
      <section className="shipment-list" aria-label="Shipment list"><h2>{operations ? 'Assigned visibility' : view === 'sending' ? 'Your outgoing shipments' : 'Your incoming shipments'}</h2>
        ...
        {view === 'receiving' && customerAccess && <div className="claim-box"><h3>Add an incoming shipment</h3><p>Your verified email and phone must match the sender’s recipient details. We then ask for a fresh email code.</p><form onSubmit={e => { e.preventDefault(); const ref = String(new FormData(e.currentTarget).get('reference')); void run(async () => { setChallenge(await command<S['ContactChallenge']>('/recipient-claims/challenges', { public_reference: ref })); setNotice('If the reference and contacts match, a code is in the local development inbox.'); }); }}><label>Shipment reference<input name="reference" required maxLength={64} placeholder="ZPX-ORDER-…" /></label><button disabled={busy}>Request claim code</button></form>
      ...
      <section ref={detail} className="shipment-detail" aria-label={creating ? 'New shipment' : 'Shipment details'}>
        {creating ? <><p className="eyebrow">NEW SHIPMENT</p><h2>Where is your parcel going?</h2><form onSubmit={create}>
          <fieldset><legend>1. Choose your lockers</legend><label>Origin locker<select name="origin" required defaultValue=""><option value="" disabled>Select origin</option>{locations.filter(l => l.draft_eligible).map(l => <option key={l.id} value={l.id}>{l.name}{l.development_only ? ' · test site' : ''}</option>)}</select></label><label>Destination locker<select name="destination" required defaultValue=""><option value="" disabled>Select destination</option>{locations.filter(l => l.draft_eligible).map(l => <option key={l.id} value={l.id}>{l.name}{l.development_only ? ' · test site' : ''}</option>)}</select></label><small>Standard locker-to-locker service. Test sites are not real drop-off locations.</small></fieldset>
          ...
          {payment?.provider === 'AUTHORIZE_NET_SANDBOX' && <div className="quote-card"><span>AUTHORIZE.NET SANDBOX · NO REAL CHARGE</span><h3>Secure test checkout</h3><p>Enter sandbox test card details on Authorize.net. Return here after checkout so ZPX can verify the payment.</p>
            {payment.checkout_token && payment.checkout_url === 'https://test.authorize.net/payment/payment' ? <form method="post" action={payment.checkout_url} target="_blank" rel="noopener noreferrer"><input type="hidden" name="token" value={payment.checkout_token} /><button className="primary" disabled={busy}>Open sandbox checkout ↗</button></form> : payment.checkout_expired ? <p>This checkout link expired. If you paid, check the receipt below. An unresolved payment needs review before another checkout.</p> : <button disabled={busy} onClick={() => void run(async () => { setPayment(await command<S['PaymentSession']>('/payments/' + payment.payment_id + '/hosted-session', {})); })}>Prepare checkout link</button>}
          ...
          <h3>Shipment history</h3>{timeline?.milestones.length ? <ol className="timeline">{timeline.milestones.map((m, i) => <li key={i}><strong>{label(m.code)}</strong><small>{date(m.occurred_at)}</small></li>)}</ol> : <p>No shipment events recorded yet.</p>}
