...
Follow-up: OpenAPI validation and generated TypeScript definitions are now implemented; see [contract evidence](M0-contracts.md). Remaining M0 includes runtime client transport, tested immutable image/toolchain pins, business-framework/reuse decision and build, mobile shells/native builds, actual serial protocol fixture tests, deployed-writer audit and terminal dependency recovery. Remaining P1.1: tracked migrations, restricted runtime DB role, domain and concurrency tests. Production deployment, complete shipping screens and ten-parcel E2E remain unimplemented.
